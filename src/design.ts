...
export async function runDesignStep(db: Db, projectId: string, targetUse: string): Promise<DesignStepOutput> {
  ...
  try {
    ...
    const llmInput = {
      rooms: rooms.map((r) => ({ label: r.label, widthM: r.width, depthM: r.height, areaM2: r.area })),
    ...
    const [cached] = await db`SELECT value FROM facts WHERE project_id = ${projectId} AND category = 'design' AND key = 'design_llm_cache_hash' AND value = ${inputHash} ORDER BY id DESC LIMIT 1`;
    if (cached) {
      const [cachedJson] = await db`SELECT value FROM facts WHERE project_id = ${projectId} AND category = 'design' AND key = 'design_llm_json' ORDER BY id DESC LIMIT 1`;
      try { if (cachedJson) llmDesign = JSON.parse(String(cachedJson.value)) as LlmDesign; } catch { llmDesign = null; }
    }
    ...
    if (llmDesign) {
      rooms = llmDesign.rooms.map((r) => ({ label: r.label, width: r.widthM, height: r.depthM, area: r.areaM2 }));
      assignments = llmDesign.rooms.flatMap((r, i) => r.zones.map((zone, zi) => ({
        zone: { id: `${r.id}-${zi}`, name: zone.label, minArea: zone.widthM * zone.depthM, zoneType: "work" as ZoneType },
        roomIndex: i, area: Math.round(zone.widthM * zone.depthM * 10) / 10, retained: !r.isNewPartition, tight: false,
      })));
      notes = llmDesign.notes;
    }
    const allocatedM2 = Math.round(assignments.reduce((acc, a) => acc + a.area, 0) * 10) / 10;
    const circulationM2 = Math.round((llmDesign?.circulationM2 ?? Math.max(0, totalArea - allocatedM2)) * 10) / 10;
    ...
    const axoSvg = renderAxonometric({ title: `CONCEPT DESIGN — ${program.label}`, address: String(proj.address), rooms, newLabels: rooms.map(() => "new"), ceilingHeightM, generatedAt: generatedAt.slice(0, 10) });
    ...
    const allNotes = [
      llmDesign ? `Concept zoning for "${program.label}" generated by ${llmProviderLabel()} from structured confidence-scored space facts (indicative concept, not construction information).` : `Concept zoning for "${program.label}" generated by the deterministic fallback from confidence-scored space facts in project memory (intelligence step).`,
      rooms.length === 0
    ...
    const factOuts: Array<{ category: string; key: string; value: string; confidence: number; sourceId: string | null }> = [
      ...
      ...(llmDesign ? [
        { category: "design", key: "design_llm_cache_hash", value: inputHash, confidence: 1, sourceId },
        { category: "design", key: "design_llm_json", value: JSON.stringify(llmDesign), confidence: 0.7, sourceId },
        { category: "design", key: "design_provider", value: llmProviderLabel(), confidence: 1, sourceId },
        { category: "design", key: "design_generation_note", value: `design generated by LLM (${llmProviderLabel()}); indicative concept, not construction information`, confidence: 0.9, sourceId },
      ] : []),
      ...
      { category: "design", key: "design_concept_axo_svg", value: axoSvg, confidence: 0.5, sourceId },
    ...
    return {
      ...
      facts: factOuts,
      sources: [{ name: "ATLAS AI concept design", url: null, notes: llmDesign ? "LLM-generated concept from structured space facts; advisory only." : "Deterministic fallback zoning concept; advisory only." }],
      design,
    };
  } catch (err) {
