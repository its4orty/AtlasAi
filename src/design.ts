...
export async function runDesignStep(db: Db, projectId: string, targetUse: string): Promise<DesignStepOutput> {
  ...
  try {
    ...
    const allNotes = [
      llmDesign ? `Concept zoning for "${program.label}" generated by Gemini 2.5 Flash from structured confidence-scored space facts (indicative concept, not construction information).` : `Concept zoning for "${program.label}" generated by the deterministic fallback from confidence-scored space facts in project memory (intelligence step).`,
      rooms.length === 0
    ...
    const factOuts: Array<{ category: string; key: string; value: string; confidence: number; sourceId: string | null }> = [
      ...
      ...(llmDesign ? [
        ...
        { category: "design", key: "design_generation_note", value: "design generated by LLM (Gemini 2.5 Flash); indicative concept, not construction information", confidence: 0.9, sourceId },
      ] : []),
    ...
    return {
      ...
      facts: factOuts,
      sources: [{ name: "ATLAS AI concept design", url: null, notes: llmDesign ? "LLM-generated Gemini concept from structured space facts; advisory only." : "Deterministic fallback zoning concept; advisory only." }],
      design,
    };
  } catch (err) {
