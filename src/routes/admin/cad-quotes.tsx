...
export const Route = createFileRoute("/admin/cad-quotes")({ server: { handlers: { GET: async ({ request }) => {
  ...
  const body = [...per.entries()]
    ...
    .map(([pid, p]) => {
      ...
      let surveySummary = "No professional survey figures";
      const rawSurvey = f.get("surveyor_dimensions")?.value;
      if (rawSurvey) { try { const s = JSON.parse(rawSurvey) as { widthMm:number; depthMm:number; notes?:string }; surveySummary = `${s.widthMm} × ${s.depthMm} mm`; if (s.notes) surveySummary += ` — ${s.notes}`; } catch { surveySummary = "Survey fact present (unreadable)"; } }
      const form = `<form method="post" action="/api/surveyor?token=${encodeURIComponent(supplied)}" style="white-space:nowrap"><input type="hidden" name="projectId" value="${esc(pid)}"/><input name="widthMm" type="number" min="1" placeholder="width mm" required style="width:90px"/><input name="depthMm" type="number" min="1" placeholder="depth mm" required style="width:90px"/><input name="notes" placeholder="notes (optional)" style="width:130px"/><button type="submit">Save survey</button></form>`;
      return `<tr><td class="strong">#${esc(pid)}</td><td>${esc(p.address)}</td><td>${contactOf(f.get("cad_quote_contact")?.value)}</td><td>${docsOf(f.get("cad_quote_docs")?.value)}</td><td>${esc(f.get("cad_quote_survey_visit")?.value ?? "—")}</td><td>${esc(f.get("cad_quote_notes")?.value ?? "—")}</td><td>${fmtDate(p.requestedFetched)}</td><td>${esc(surveySummary)}<br/><small>Accurate CAD source: ${rawSurvey ? "professional surveyor figures" : "client document, then demo"}</small><br/>${form}</td><td>${unlocked ? '<span style="background:#e8efec;border:1px solid #9db8ab;color:#2e5c46;padding:2px 8px;border-radius:3px;font-weight:700">UNLOCKED</span>' : '<span style="background:#f7efe3;border:1px solid #c98a4a;color:#8a5a1e;padding:2px 8px;border-radius:3px;font-weight:700">locked — not released</span>'}</td></tr>`;
    })
    .join("");
